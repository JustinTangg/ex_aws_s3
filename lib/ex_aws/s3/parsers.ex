if Code.ensure_loaded?(SweetXml) do
  defmodule ExAws.S3.Parsers do
    ...
    @spec parse_all_my_buckets_result(any) :: any
    ...
    @spec parse_initiate_multipart_upload(any) :: any
    ...
    def parse_bucket_policy_status({:ok, resp = %{body: xml}}) do
      parsed_body =
        xml
        |> SweetXml.xpath(~x"//PolicyStatus",
          public: ~x"./IsPublic/text()"s |> to_boolean()
        )
    ...
    @spec parse_bucket_object_versions({:ok, %{body: any}}) :: {:ok, %{body: nil | [any] | map}}
  ...
  defmodule ExAws.S3.Parsers do
    ...
    def parse_bucket_policy_status(_val), do: missing_xml_parser()
